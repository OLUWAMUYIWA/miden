...
/// - `ctx` contains context ID. Values in this column must increase monotonically but there
///   can be gaps between two consecutive context IDs of up to 2^32. Also, two consecutive values
///   can be the same.
...
pub struct Memory {
    ...
    /// Memory segment traces sorted by their context ID.
    trace: BTreeMap<u32, MemorySegment>,
...
impl Memory {
    ...
    /// Returns a word located at the specified address of the specified memory segment, or None if
    /// the address hasn't been accessed previously.
    ///
    /// Unlike read() which modifies the memory access trace, returns the value at the specified
    /// address (if one exists) without altering the memory access trace.
    ...
    pub fn read(&mut self, ctx: u32, addr: Felt) -> Word {
        self.num_trace_rows += 1;
        let clk = Felt::from(self.clk);

        self.trace
            .entry(ctx)
            .or_insert_with(MemorySegment::default)
            .read(addr, clk)
    }

    /// Writes the provided word (4 elements) at the specified address.
    pub fn write(&mut self, ctx: u32, addr: Felt, value: Word) {
        self.num_trace_rows += 1;
        let clk = Felt::from(self.clk);
